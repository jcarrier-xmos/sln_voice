...
static void led_task(void *args)
{
    ...
    while (1) {
        ...
        if (notif_value & TASK_NOTIF_MASK_ASLEEP) {
            // VNR is not detecting speech, in low power mode.
            xTimerStop(tmr_hbeat, 0);
            color = LED_RED;
            state = LED_ON;
        } else if ((notif_value & TASK_NOTIF_MASK_AWAKE) ||
                   (notif_value & TASK_NOTIF_MASK_WAITING)) {
            // VNR is detecting speech, normal operation (heartbeat)
            color = LED_GREEN;
            state = LED_TOGGLE;
            xTimerStart(tmr_hbeat, 0);
        } else if (notif_value & TASK_NOTIF_MASK_LISTEN) {
            // VNR is detecting speech, wake word detected, listening for command
            xTimerStop(tmr_hbeat, 0);
            color = LED_YELLOW;
            state = LED_ON;
        }
