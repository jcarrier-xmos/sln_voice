...
#define PRINT_PS_REG(x)  debug_printf("[%d] : " #x ":\t\t0x%08lX\n", THIS_XCORE_TILE, getps(x))
#define PRINT_PSWITCH_REG(x)  do { unsigned tmp; read_pswitch_reg(get_local_tile_id(), x, &tmp); debug_printf("[%d] : " #x ":\t\t0x%08lX\n", THIS_XCORE_TILE, tmp); } while (0)
#define PRINT_SSWITCH_REG(x)  do { unsigned tmp; read_sswitch_reg(get_local_tile_id(), x, &tmp); debug_printf("[%d] : " #x ":\t\t0x%08lX\n", THIS_XCORE_TILE, tmp); } while (0)
...
void platform_init(chanend_t other_tile_c)
{
    ...
    SYNC();
    debug_printf("clock_control_init (tile %d on core %d)\n", THIS_XCORE_TILE, portGET_CORE_ID());
    clock_control_init();
    ...
    SYNC();
    debug_printf("uart_init (tile %d on core %d)\n", THIS_XCORE_TILE, portGET_CORE_ID());
    uart_init();
}
